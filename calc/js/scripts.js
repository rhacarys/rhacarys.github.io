...
const calcular = () => {
    ...
    console.log(parcels + ' parcelas: ' + percent + '%');

    debugger;
    let increment = (percent / 100) * value;
    let result = (value + increment) / (parcels || 1);

    $('#result').html(isNaN(result) ? '-' : 'R$ ' + result.toFixed(2).replace('.', ','));
}

$(document).ready(() => {
    data.map(item => $('#parcel').append(item.title, `<option value="${item.key}">${item.title}</option>`));
});